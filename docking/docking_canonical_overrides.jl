...
function set_charge(ta::TrjArray{T,U}) where {T,U}
    ...
    for iatom = 1:ta.natom
        ...
        if ta.atomname[iatom] == "N"
            ...
            # ATOM TYPE "O"
            # B3 fix: was `ta.resname[iatom] == "O"/"OXT"` which is dead code
            # (residue names are 3-letter codes like "ALA", never "O"). The
            # check should be against the atom name. Using the local
            # `atomname` (= deepcopy of ta.atomname) to match the style of
            # the side-chain branches below.
        elseif atomname[iatom] == "O"
            charge[iatom] = 10
        elseif atomname[iatom] == "OXT"
            charge[iatom] = 2
...
function assign_Re_potential!(grid_imag::AbstractArray{T},
    ...
    # B1 fix: was `grid_imag = grid_charge ./ grid_dis`, which only rebinds the
    # local name and leaves the caller's array untouched. Broadcast-assign so
    # the caller sees the computed potential, and guard against division by
    # zero (grid cells outside rcut have grid_dis == 0 → potential is 0).
    grid_imag .= ifelse.(grid_dis .> zero(T), grid_charge ./ max.(grid_dis, eps(T)), zero(T))
...
function assign_Li_potential!(grid_imag::AbstractArray{T},
    ...
    # B1 fix: broadcast-assign + division-by-zero guard (see
    # assign_Re_potential! for details).
    grid_imag .= ifelse.(grid_dis .> zero(T), grid_charge ./ max.(grid_dis, eps(T)), zero(T))
...
function ChainRulesCore.rrule(::typeof(docking_score_elec), receptor_org::TrjArray{T,U}, ligands_org::TrjArray{T,U}, alpha::T, iface_ij::AbstractVector{T}, beta::T, charge_score::AbstractArray{T}) where {T,U}
    # B5 fix: was `spacing = 1.5`, inconsistent with the forward pass
    # (`docking_score_elec` uses spacing=3.0). The backward was computing
    # gradients of a different (finer-grid) scoring function, which made the
    # rrule disagree with finite differences by ~8x on dL/dα. Also was calling
    # `MDToolbox.generate_grid` (the docking.jl version) while the forward
    # calls the local overridden `generate_grid`; switch to the local one so
    # forward and backward traverse identical code paths.
    spacing = 3.0
    receptor = deepcopy(receptor_org)
    ligands = deepcopy(ligands_org)
